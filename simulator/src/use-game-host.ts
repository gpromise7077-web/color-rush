...
export function useGameHost(
  ...
  const methods = useMemo<HostApiV1>(
    () => ({
      ...
      async openSession({ wager, gameData, randomnessRequestData }) {
        requireReadyWallet();
        ...
        const actions: BatchAction[] = [
          { target: runtime.token, value: 0n, data: encodeApprove(runtime.proxy, wagerWei) },
          {
            ...
            data: encodeOpenSession({
              ...
              wager: wagerWei,
              gameData,
              randomnessRequestData: randomnessRequestData ?? EMPTY_HEX,
            }),
          },
        ];
      ...
      async submitAction({ sessionId, actionData, randomnessRequestData, approvalAmount }) {
        requireReadyWallet();
        ...
        try {
          const { transactionHash } = await runtime.sendActions(actions);
        ...
        const { transactionHash } = await runtime.sendActions([
          {
            target: runtime.proxy,
            value: 0n,
            data: encodeCancelStuckRandomness(BigInt(sessionId)),
          },
        ]);
        return { transactionHash };
      },

...
// a different identity than ours — the boundary is typed loosely on purpose.
type LocalVerifyNetworkModule = {
  LOCAL_VRF_NODE_PRIVATE_KEY: Hex;
  setupLocalVerifyNetwork(input: {
    publicClient: unknown;
    walletClient: unknown;
  }): Promise<Address>;
  depositLocalVrfClientBalance(input: {
    publicClient: unknown;
    walletClient: unknown;
  ...
  startLocalVerifyNetworkNode(input: {
    publicClient: unknown;
    walletClient: unknown;
    routerAddress: Address;
  }): void;
...
async function loadLocalVerifyNetwork(): Promise<LocalVerifyNetworkModule> {
  try {
    return (await import('../../local-verify-network/src/index.ts')) as unknown as LocalVerifyNetworkModule;
  } catch (error) {
    console.error(
...
async function main() {
  ...
  await verifyNetwork.depositLocalVrfClientBalance({
    publicClient,
